...
#from config.ParticleNet import ParticleNet, FeatureConv
import numpy as np
...
def get_graph_feature_v1(x, k, idx):
    ...
    idx = idx + idx_base # still (batch_size, num_points, k)
    ...
    # fts[idx, :]: (idx.shape, num_dims) = (batch_size * num_points * k, num_dims)
    fts = fts[idx, :].view(batch_size, num_points, k, num_dims)  # neighbors: -> (batch_size*num_points*k, num_dims) -> ...
    # at this point fts: (batch_size, num_points, k, num_dims)
...
class EdgeConvBlock(nn.Module):
    ...
    def __init__(self, k, in_feat, out_feats, batch_norm=True, activation=True, cpu_mode=False):
        ...
        for i in range(self.num_layers):
            # 2 * in_feat: x_i features + (x_i - x_j) features
            ...
            # kernel_size = 1, rest set to default: H_in = H_out, W_in = W_out
            # (batch_size, in_feat, num_points, k) -> (batch_size, out_feats, num_point, k)
    ...
    def forward(self, points, features):
        ...
        topk_indices = knn(points, self.k) # (batch_size, num_points, k)
